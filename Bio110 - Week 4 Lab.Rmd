...
# NOTE!!!! Just starting with week 3 rmd, and will modify to deal with groups (more topics if necessary)
...
myStats<-data %>% group_by(time) %>% summarise (avg=mean(length), sd=sd(length))
...
group_by(.data=data, time) %>% dplyr::summarize(avg=mean(length), stdDev=sd(length), n=n(), stdEr=sd(length)/sqrt(n())) %>% mutate(se = stdDev / sqrt(n),
         lower_ci = lower_ci(avg, se, n),
         upper_ci = upper_ci(avg, se, n))
