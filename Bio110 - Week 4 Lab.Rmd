...
myData<-read_csv("chlamydataforR.csv")
...
Go ahead and try the group_by/summarise technique with your data, by copying and pasting the code in line 64 to line 89, then run the code block.
...
ggplot(data=myStats, mapping=aes(x=time,y=Average))+geom_point()+geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI),width=5)+geom_path(mapping=aes(x=time,y=Average))+xlim(0,90)+ylim(0,8)
...
ggplot(data=myStats, mapping=aes(x=time,y=Average))+geom_point()+geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI),width=5)+geom_path(mapping=aes(x=time,y=Average))+xlim(0,90)+ylim(0,8)+labs(x="time in minutes", y="flagella length in micrometers")
