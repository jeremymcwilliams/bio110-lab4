...
#myStats<-myData %>% group_by(time) %>% summarise (Average=mean(length), StandardDeviation=sd(length), n=n(), lowerCI<-myMean-((qnorm(0.975)*myStdDev)/sqrt(n)), upperCI<-myMean+((qnorm(0.975)*myStdDev)/sqrt(n)))
...
ggplot(data=myStats, mapping=aes(x=time, y=avg))+geom_point()

...where myStats is your new data set (after running the groub_by/summarise code), time is your column header for the time group, and avg is the column header for the mean.
...
# ggplot(data=myStats, mapping=aes(x=time, y=avg))+geom_point()++geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI), width=5)
...
Now let's add the line connecting the dots using geom_path(). We can do this by appending  +geom_path(mapping=aes(x=time, y=avg)), making sure "time" and "avg" match your column headings. 

```{r}

# Add the line to the graph:
# ggplot(data=myStats, mapping=aes(x=time, y=avg))+geom_point()++geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI), width=5)+geom_path(mapping=aes(x=time, y=avg))


```


Now we can clean up the plot by adjusting our x- and y- axes with the xlim() and ylim() functions. Each function takes a minimum and maximum (e.g. +ylim(0, 100))

```{r}

# Adjust your axes with the xlim and ylim functions. Edit the values those functions below to improve your plot.

# ggplot(data=myStats, mapping=aes(x=time, y=avg))+geom_point()++geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI), width=5)+geom_path(mapping=aes(x=time, y=avg))+xlim(0,100)+ylim(0,100)
...
#ggplot(data=myStats, mapping=aes(x=time, y=avg))+geom_point()++geom_errorbar(mapping=aes(ymin=lowerCI, ymax=upperCI), width=5)+geom_path(mapping=aes(x=time, y=avg))+xlim(0,100)+ylim(0,100)+labs(title="my title", x="my x label", y="my y axis label")
